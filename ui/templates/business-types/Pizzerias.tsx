...
const ICONS = [ItemManagementIcon, ManagementIcon, UserFriendlyIcon, OnsiteSupportIcon];
  ...
  return (
    <div>
      <Banner
        ...
        content={<span>{common('business_categories.pizza')}</span>}
        button={
          <GetPricingButton
      ...
      <BreadCard isGrey>
        <SolutionList
          headings={[
            ...
            ` ${common('pos_system_for')} ${common('business_categories.pizza')}`,
          ]}
          content={[
            {
      ...
      <BreadCard>
        <Heading level={3}>{`${common('business_categories.pizza')} ${common(
          'must_have',
        )}`}</Heading>
